...
public class BankCardParse {
    ...
    synchronized public BankResponse parseResponse(BankResponse icResponse, String lastMainCardNo, long lastTime, int amount, String aid) throws Exception {
        ...
        icResponse = syncSSLRequest.request(UnionConfig.PAY_TYPE_BANK_IC, sendData);
    ...
    private void saveUnionPayEntity(int amount, String mainCardNo, String tlv, byte[] sendData, String cardNum) {
        ...
        payEntity.setBusNo(carConfig.getBusNo());//车辆号
        payEntity.setBus_line_name(useConfig.getLine_chinese_name());//线路中文名
        payEntity.setBus_line_no(useConfig.getLine());//线路号
        payEntity.setDriverNum(useConfig.getDriverNo());//司机号
        payEntity.setUnitno(carConfig.getUnionTraNo());//分公司号
        /********************************************************/
