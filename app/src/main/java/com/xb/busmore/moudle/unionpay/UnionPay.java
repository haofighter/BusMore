...
import static com.xb.busmore.moudle.unionpay.unionutil.ParseUtil.parseMackey;
                    ...
                    try {
                        ...
                        if (what == UnionConfig.SIGN) {//签到
                            if (message0810.getValue(39).getValue().equals("00")) {
                                ...
                                if (isTip) {
                                    BusToast.showToast(App.getInstance().getApplicationContext(), "银联签到失败[" + message0810.getValue(39).getValue() + "]", false);
                                }
                            }
                            ...
                            if (unique != null) {
                                ...
                                switch (resCode) {
                                    ...
                                    case "A6":
                                        ...
                                        SoundPoolUtil.play(UnionConfig.IC_BASE2);
                                        String amount = message0810.getValue(4).getValue();
                                        BusToast.showToast(App.getInstance().getApplicationContext(), "扣款成功\n扣款金额" + yuan2Fen(amount) + "元", true);
                                        SLog.d("UnionPay(success.java:104)修改成功");
                                        break;
                                    case "A0":
                                        //重新签到
                                        BusToast.showToast(App.getInstance().getApplicationContext(), "刷卡失败,正在签到,稍后重试", false);
                                    ...
                                    case "94":
                                        //重复交易（流水号重复）
                                        BusToast.showToast(App.getInstance().getApplicationContext(), "刷卡失败,流水重复,请重试", false);
                                        SLog.d("UnionPay(success.java:104)重复支付");
                                        break;
                                    case "51":
                                        //余额不足
                                        SoundPoolUtil.play(UnionConfig.EC_BALANCE);
                                        BusToast.showToast(App.getInstance().getApplicationContext(), "刷卡失败,余额不足", false);
                                        SLog.d("UnionPay(success.java:130)余额不足");
                                        break;
                                    case "54":
                                        //卡过期
                                        SoundPoolUtil.play(UnionConfig.IC_INVALID);
                                        BusToast.showToast(App.getInstance().getApplicationContext(), "卡过期", false);
                                        SLog.d("UnionPay(success.java:136)卡过期");
                                        break;
                                    default:
                                        BusToast.showToast(App.getInstance().getApplicationContext(), "刷卡失败[" + Util.unionPayStatus(resCode) + "]", false);
                                        break;
                                }
                            }
                            dao.update(unique);
                        ...
                        e.printStackTrace();
                        BusToast.showToast(App.getInstance().getApplicationContext(), "刷卡失败[异常]\n" + e.toString(), false);
                    }
                }

                @Override
                public void fail(int what, String e) {
                    BusToast.showToast(App.getInstance().getApplicationContext(), "刷卡失败[超时]", false);
                }
            });
        }
    }
